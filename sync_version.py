...
def update_app(version: str) -> None:
    path = TARGETS["app"]
    content = path.read_text(encoding="utf-8")
    content = replace_regex(
        content,
...
def read_versions() -> dict:
    ...
    app_text = TARGETS["app"].read_text(encoding="utf-8")
    ...
    app_v = re.search(r'^VERSION\s*=\s*"([0-9.]+)"\s*$', app_text, flags=re.MULTILINE)
    ...
    return {
        ...
        "app.py": app_v.group(1) if app_v else None,
