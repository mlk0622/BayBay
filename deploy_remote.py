...
try:
    ...
    step1_commands = [
        # Mettre à jour les paquets et installer les dépendances (y compris ClamAV antivirus)
        ...
        # Nettoyer l'ancien dossier et cloner le repo
    ...
    step2_commands = [
        ...
        # Synchroniser mb-site sur /var/www/html
...
EOF""",
    ...
    success = run_ssh_commands(client, step1_commands)
    if not success:
        print("\n❌ Échec de la préparation.")
        client.close()
        sys.exit(1)

    # 2. Transférer le fichier .env chiffré/sécurisé via SFTP
    ...
    # 3. Exécuter le reste des commandes
    success = run_ssh_commands(client, step2_commands)
    if success:
